...
*최종 업데이트: 2026-04-16 — Phase 2.1/2.2: `v-model:checkedIds`/`v-model:filters`/`v-model:sort` 양방향 바인딩 지원(제어 모드), 하위 호환으로 기존 `@update:checked`(행 객체 배열)·`@sort` 이벤트 유지, 개발 모드(`import.meta.env.DEV`) 상호배제 경고 추가(useTree+groupBy / useServerSide+useTree / 병합+대량 데이터), `controlled-state.md` 가이드 신규 생성 및 사이드바 등록, `update:checkedIds`/`update:sort` 이벤트 문서화.*
