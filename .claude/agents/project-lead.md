...
- 작업 특성에 맞는 에이전트를 선택하고 **작업 지시서를 작성하여 반환**
- **절대 직접 Agent 도구로 에이전트를 spawn하지 않는다** — 에이전트 spawn은 메인 어시스턴트가 담당
- 각 에이전트에 전달할 프롬프트(작업 지시서)를 명확하게 작성하여 보고에 포함
- 병렬 실행 가능한 작업과 순차 실행 작업을 구분하여 명시
- 에이전트 결과를 수집하고 통합
