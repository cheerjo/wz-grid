...
| 컬럼 타입 | `#column-types` | 17종 컬럼 타입 인터랙티브 데모 및 이벤트 로그 확인 |

상단 **라이선스 키** 입력란에 Pro 키를 입력하면 Pro 전용 기능(고급 필터, 행 드래그, 그룹핑 등)을 데모 앱에서 즉시 활성화할 수 있습니다. **데모키 입력** 버튼을 누르면 테스트용 Pro 키가 자동으로 채워집니다.
