...
| `virtualizeWithMerge` | — | `boolean` | `false` | 병합(autoMergeCols / mergeCells) 활성 상태에서도 가상 스크롤 유지. 병합 블록 경계까지 range를 자동 확장해 블록이 절단되지 않음. 대량 데이터 + 병합 시나리오의 성능 개선 opt-in |
...
*최종 업데이트: 2026-04-17 — Phase 4.4 (병합 + 가상 스크롤 부분 지원): `useMerge`에 `getMergeSpan(itemIdx)` 추가 — 행이 속한 가장 넓은 병합 블록의 `[start, end)` 범위 반환(여러 컬럼 병합이 겹치면 더 넓은 쪽으로 자동 확장). `WZGrid.virtualizeWithMerge` prop 신규(기본 false, opt-in) — true 설정 시 `_vs.visibleRange`를 병합 블록 경계까지 확장해 **병합이 활성인 상태에서도 viewport 밖 행은 렌더하지 않음**. 기존 동작(병합=전체 렌더)은 default로 유지. topPadding/bottomPadding도 확장된 range 기반으로 재계산. `useMerge` 호출 위치를 가상 스크롤 computed보다 앞으로 이동(선언 순서 정리). `useMerge` 4종 getMergeSpan 테스트 추가(총 66 테스트 통과). `MergeRowSpan` 타입 export.*

*이전 업데이트: 2026-04-17 — Phase 4.3 (Undo/Redo 편집 히스토리): `useUndoRedo` composable 신규 추가 — LIFO undo/redo 스택, 분기 변경 시 redo 자동 비움, `maxDepth` FIFO drain. WZGrid에 `useUndo`/`maxUndoDepth` props 추가, `Ctrl+Z`/`Cmd+Z`는 Undo, `Ctrl+Y`/`Ctrl+Shift+Z`/`Cmd+Shift+Z`는 Redo로 바인딩. 단방향 데이터 흐름 유지를 위해 Undo/Redo 재생 시 `rows`를 직접 mutate 하지 않고 `@update:cell` 이벤트를 역/재 값으로 재 emit — 부모 핸들러 하나로 편집/언두/리두 모두 처리. `@undo`/`@redo` 선택적 이벤트(HistoryEntry 페이로드) 신규 emit. `CellUpdateEvent<T>` 타입에 `rowIdx`·`colKey`·`oldValue` 공식화, 기존 `key` 필드는 `colKey`의 하위 호환 별칭으로 유지. `useUndoRedo` export, `tests/composables/useUndoRedo.spec.ts` 8종 테스트 추가 (총 62 테스트 통과).*
