...
컬럼 헤더 드래그 앤 드롭 순서 변경을 관리합니다. **WZGrid 내부에서 자동 처리되므로** 별도 핸들러 없이도 동작합니다.
...
| 매개변수 | 타입 | 설명 |
| :--- | :--- | :--- |
| `getColumns` | `() => Column[]` | 컬럼 배열 getter |
| `onReorder` | `(srcKey, targetKey) => void` | 재배치 완료 시 콜백 (`@reorder:columns` 이벤트 트리거용) |
| `isResizing` | `() => boolean` | 리사이즈 중 여부. `true`이면 드래그 차단 |

### 반환값

| 속성 | 타입 | 설명 |
| :--- | :--- | :--- |
| `reorderedColumns` | `ComputedRef<Column[]>` | 드래그 순서가 적용된 컬럼 배열 |
| `dragSourceColIdx` | `Ref<number>` | 드래그 소스 인덱스 |
| `dragOverColIdx` | `Ref<number>` | 드래그 오버 인덱스 |
| `onDragStart` / `onDragOver` / `onDragLeave` / `onDrop` / `onDragEnd` | 이벤트 핸들러 | 드래그 앤 드롭 이벤트 처리 함수 |

---

## useRowDragDrop

행 드래그 앤 드롭 순서 변경을 관리합니다. **WZGrid 내부에서 자동 처리되므로** 별도 핸들러 없이도 동작합니다.

### 시그니처

```ts
function useRowDragDrop(
  getRow: (idx: number) => any,
  onReorder: (from: any, to: any, position: 'above' | 'below') => void,
  getRows: () => any[]
): UseRowDragDropReturn
```

| 매개변수 | 타입 | 설명 |
| :--- | :--- | :--- |
| `getRow` | `(idx: number) => any` | 인덱스로 행 데이터 조회 |
| `onReorder` | `(from, to, position) => void` | 재배치 완료 시 콜백 (`@reorder:rows` 이벤트 트리거용) |
| `getRows` | `() => any[]` | 현재 정렬된 행 배열 getter |

### 반환값

| 속성 | 타입 | 설명 |
| :--- | :--- | :--- |
| `reorderedRows` | `ComputedRef<any[]>` | 드래그 순서가 적용된 행 배열 |
| `rowDragSrcIdx` | `Ref<number>` | 드래그 소스 인덱스 |
| `rowDragOverIdx` | `Ref<number>` | 드래그 오버 인덱스 |
| `rowDragOverPos` | `Ref<'above' \| 'below'>` | 드래그 위치 (위/아래) |
| `onRowDragStart` / `onRowDragOver` / `onRowDrop` / `onRowDragEnd` | 이벤트 핸들러 | 드래그 앤 드롭 이벤트 처리 함수 |
