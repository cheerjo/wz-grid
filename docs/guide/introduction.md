...
### Excel 완벽 연동
셀 범위 선택 후 Ctrl+C로 복사하고 Excel에 붙여넣을 수 있습니다. 반대로 Excel에서 Ctrl+C한 데이터를 그리드에 붙여넣기도 가능합니다. `.xlsx` 파일 직접 내보내기도 지원합니다.
...
프로젝트를 클론한 후 `npm run dev`를 실행하면 탭 기반 인터랙티브 데모 앱이 열립니다.

| 탭 | URL 해시 | 내용 |
|:---|:---------|:-----|
| 종합 데모 | `#basic` | 편집, 정렬, 필터, 그룹핑, 페이징 등 핵심 기능 종합 체험 |
| 트리 그리드 | `#tree` | `useTree` prop을 사용한 계층형 데이터 트리 뷰 |
| 컬럼 타입 | `#column-types` | 19종 컬럼 타입 인터랙티브 데모 및 이벤트 로그 확인 |
