...
> **패키지명 변경 안내 (v1.5.0)**
>
> `wz-grid` 패키지는 deprecate되었습니다. 이후 버전은 아래 패키지로 분리 배포됩니다.
>
> | 패키지 | 용도 |
> | :--- | :--- |
> | `@wezon/wz-grid-vue` | Vue 2 / 3 래퍼 (기존 `wz-grid`의 후속) |
> | `@wezon/wz-grid-react` | React 래퍼 |
> | `@wezon/wz-grid-core` | 프레임워크 무관 순수 TS 코어 |
>
> 기존 `wz-grid` 사용자는 `npm install @wezon/wz-grid-vue`로 교체하고 import 경로만 바꾸면 됩니다.
...
import type { Column, GridRow } from '@wezon/wz-grid-vue';
...
*최종 업데이트: 2026-05-04 — 모노레포 전환 반영: `wz-grid` → `@wezon/wz-grid-vue` 패키지명 변경 안내 추가. `@wezon/wz-grid-react`(React 래퍼), `@wezon/wz-grid-core`(프레임워크 무관 순수 TS 코어) 패키지 추가. 빠른 시작 예시의 import 경로 갱신.*
