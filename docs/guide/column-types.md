...
| `tag` | 편집 불가 | 텍스트 포함 여부 |
| `sparkline` | 편집 불가 | — |
| `textarea` | 더블 클릭 / Enter → 확장 팝업, Enter 줄바꿈, Ctrl+Enter 완료 | 텍스트 부분 일치 |
...
`sparklineType`으로 line, area, bar, column 4가지 차트 스타일을 선택할 수 있습니다.
...
- **편집 중 Enter**: 셀 내부 자연스러운 줄바꿈 지원
- **편집 완료**: 하단 `[확인]` 버튼 클릭 또는 `Ctrl+Enter` 단축키 입력
- **편집 취소**: 하단 `[취소]` 버튼 클릭 또는 `Esc` 키 입력
